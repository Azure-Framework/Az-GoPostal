...
local function dbg(src, msg)
  ...
  print(("[az_gopostal][debug][%s] %s"):format(tostring(src), tostring(msg)))
end
...
_G['AZ_GOPOSTAL_SV'] = _G['AZ_GOPOSTAL_SV'] or {}
local SV = _G['AZ_GOPOSTAL_SV']
SV.dbg = dbg
SV.getJob = getJob
...
RegisterCommand('az_gopostaldebug', function(source)
  local src = source
  if src == 0 then
    print("[az_gopostal] use this in-game")
    return
  end
  local j = getJob(src)
  dbg(src, ("job=%s"):format(j))
  TriggerClientEvent('az_gopostal:notify', src, ("[az_gopostal] job=%s (see server console)"):format(j))
end, false)
...
RegisterCommand('quitjob', function(source)
  ...
  setJob(src, "unemployed", function(ok4, how)
    if ok4 then
      ...
      TriggerClientEvent('az_gopostal:notify', src, "You quit your job. (unemployed)")
    else
      dbg(src, "quitjob FAIL: " .. tostring(how))
      TriggerClientEvent('az_gopostal:notify', src, "Could not quit job (missing setter/DB). Use Job Center.")
    end
  end)
end, false)

RegisterNetEvent('az_gopostal:requestStart', function()
  local src = source
  local ok = _G['AZ_GOPOSTAL_SV'].ensureJob(src)
  if not ok then
    TriggerClientEvent('az_gopostal:notify', src, "You are not employed for GoPostal.")
    TriggerClientEvent('az_gopostal:startDenied', src)
    return
  end
  TriggerClientEvent('az_gopostal:startOk', src)
end)

RegisterNetEvent('az_gopostal:stopComplete', function()
  local src = source
  local ok = _G['AZ_GOPOSTAL_SV'].ensureJob(src)
  if not ok then return end
  exports[Config.FrameworkResource]:addMoney(src, Config.PayPerStop or 120)
end)
